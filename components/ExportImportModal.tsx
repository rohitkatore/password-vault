...
export default function ExportImportModal({ vaultItems, onImport, onClose }: ExportImportModalProps) {
  ...
  const handleExportJSON = async () => {
    try {
      ...
      alert(`… Exported ${decryptedItems.length} items successfully!`);
    } catch {
  ...
  const handleExportCSV = async () => {
    try {
      ...
      alert(`… Exported ${decryptedItems.length} items to CSV successfully!`);
    } catch {
      alert('Failed to export vault data');
    }
  };
              ...
              {importSuccess && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <p className="text-sm text-green-800">
                    … Import successful! Items are being added to your vault...
                  </p>
                </div>
              )}
